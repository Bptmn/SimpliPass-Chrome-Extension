...
import { PopoverCredentialPicker } from 'app/content/popovers/PopoverCredentialPicker';
import { decryptData } from 'shared/utils/crypto';
import {
  getRootDomain,
  getRegisteredDomain,
  matchesCredentialDomainOrTitle,
} from 'shared/utils/domain';
import { useCredentialInjection } from 'shared/utils/valuesInjection';
...
function cleanupLoginFieldListeners(): void {
  const fields = getLoginRelatedFields();
  fields.forEach((field) => {
    field.removeEventListener('click', handleFieldFocusOrClick);
  });
}
