...
import { openDB, putItem, getItem, deleteItem, clearStore } from 'services/storage/indexdb';
import { deriveKey } from 'shared/utils/crypto';
...
export async function getUserSecretKey(): Promise<string | null> {
  ...
  console.log(
    '[User] getUserSecretKey success:',
    result ? 'Key found: ' + result : 'Key not found',
  );
  return result;
}
