...
import { getAllCredentialsInFirestore } from 'services/api/firestore';
import { encryptData, decryptData } from 'shared/utils/crypto';
import { getUserSecretKey } from 'features/auth/services/user';
import { CredentialDecrypted, CredentialVaultDb } from 'shared/types/types';
import { openDB, putItem, getAllItems, getItem, deleteItem } from 'services/storage/indexdb';
import { decryptAllCredentials } from 'services/crypto/cryptography';
import { encryptCredential } from 'services/crypto/cryptography';
import {
  createCredentialInFirestore,
  updateCredentialInFirestore,
  deleteCredentialInFirestore,
} from 'services/api/firestore';
import { getRegisteredDomain } from 'shared/utils/domain';
...
export async function refreshCredentialsInVaultDb(
  ...
  const itemsToStoreInIndexDb: CredentialVaultDb[] = decrypted.map((cred) => {
    ...
    return {
      ...
      passwordCipher,
      note: cred.note || '',
      document_reference_id: cred.document_reference.id,
    };
  });
  ...
  console.log(
    '[Items] refreshCredentialsInVaultDb success:',
    itemsToStoreInIndexDb.length,
...
export async function getAllCredentialsFromVaultDbWithFallback(
  ...
  creds.forEach((cred) => {
    console.log(
      `[Cache] Credential ID: ${cred.document_reference_id}, URL: ${cred.url}, Title: ${cred.title}`,
    );
  });

  console.log(
    '[Items] getAllCredentialsFromVaultDbWithFallback success:',
    creds.length,
    'credentials',
  );
  return creds;
...
export async function saveCredentialsInVaultDb(cached: CredentialVaultDb[]): Promise<void> {
  console.log('[Items] Saving credentials to vault DB:', cached.length, 'credentials');
  const db = await openDB(DB_NAME, DB_VERSION);
  for (const cred of cached) {
    console.log('[Items] Saving credential to vault DB:', cred.document_reference_id);
    await putItem(db, STORE_NAME, cred.document_reference_id, cred);
  }
  console.log('[Items] saveCredentialsInVaultDb success');
}

/* Retrieves all cached credentials from IndexedDB */
export async function getAllCredentialsFromVaultDb(): Promise<CredentialVaultDb[]> {
...
export async function getCredentialFromVaultDb(id: string): Promise<CredentialVaultDb | null> {
  console.log('[Items] Getting credential from vault DB with ID:', id);
  const db = await openDB(DB_NAME, DB_VERSION);
  const result = await getItem(db, STORE_NAME, id);
  console.log(
    '[Items] getCredentialFromVaultDb success:',
    result ? 'Credential found' : 'Credential not found',
  );
  return result;
}
...
export async function getCredentialsByDomainFromVaultDb(
  ...
  console.log(
    '[Items] getCredentialsByDomainFromVaultDb success:',
    filtered.length,
    'credentials found for domain:',
    domain,
  );
  return filtered;
}
...
export async function createCredential(cred: CredentialDecrypted): Promise<void> {
  ...
  console.log('[ENCRYPTION] itemKey:', cred.itemKey);
  console.log('[ENCRYPTION] password (plain):', cred.password);
  console.log('[ENCRYPTION] encrypted content:', encrypted.content_encrypted);
  console.log('[ENCRYPTION] encrypted itemKey:', encrypted.item_key_encrypted);
  // 2. Store in Firestore
  const docRef = await createCredentialInFirestore(user.uid, encrypted);
  // 3. Add to IndexedDB
  const db = await openDB(DB_NAME, DB_VERSION);
  // Use the docRef.id as the key
  const credForIndexDb: CredentialVaultDb = {
    url: cred.url,
    title: cred.title,
    username: cred.username,
  ...
  await putItem(db, STORE_NAME, docRef.id, credForIndexDb);
}

// Update a credential: encrypt, update in Firestore, then update in IndexedDB if successful.
export async function updateCredential(cred: CredentialDecrypted): Promise<void> {
  const userSecretKey = await getUserSecretKey();
  const user = auth.currentUser;
  if (!userSecretKey || !user) throw new Error('User not authenticated');
  // 1. Encrypt the credential
  ...
  await updateCredentialInFirestore(user.uid, cred.document_reference.id, encrypted);
  ...
  await putItem(db, STORE_NAME, cred.document_reference.id, credForIndexDb);
  console.log('[Items] updateCredential success');
}

// Delete a credential: remove from Firestore, then from IndexedDB if successful.
export async function deleteCredential(cred: CredentialDecrypted): Promise<void> {
  const user = auth.currentUser;
  if (!user) throw new Error('User not authenticated');
  // 1. Delete from Firestore
  await deleteCredentialInFirestore(user.uid, cred.document_reference.id);
  // 2. Delete from IndexedDB
  const db = await openDB(DB_NAME, DB_VERSION);
  await deleteItem(db, STORE_NAME, cred.document_reference.id);
}
