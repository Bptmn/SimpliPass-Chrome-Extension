...
import { User, CredentialEncrypted } from '@shared/types';
...
export async function createCredentialInFirestore(
  ...
  const { document_reference: _document_reference, ...dataToStore } = data;
...
export async function updateCredentialInFirestore(
  ...
  const credRef = doc(db, 'users', userId, 'my_credentials', credentialId);
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const { document_reference: _document_reference, ...dataToUpdate } = data;
 ...
 */
export async function addDocument<T extends Record<string, unknown>>(collectionPath: string, data: T) {
 ...
 */
export async function setDocument<T extends Record<string, unknown>>(docPath: string, data: T) {
 ...
 */
export async function updateDocument<T = Record<string, unknown>>(docPath: string, data: Partial<T>) {
  const docRef = fsDoc(db, docPath);
 ...
 */
export async function getDocument<T = Record<string, unknown>>(docPath: string): Promise<T | null> {
  const docRef = fsDoc(db, docPath);
 ...
 */
export async function getCollection<T = Record<string, unknown>>(collectionPath: string, ...queryConstraints: QueryConstraint[]): Promise<T[]> {
  const colRef = fsCollection(db, collectionPath);
