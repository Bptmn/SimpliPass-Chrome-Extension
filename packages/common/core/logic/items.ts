...
import { db } from '@common/core/adapters/database.adapter';
import { decryptAllItems, encryptCredential, encryptBankCard, encryptSecureNote } from '@common/core/logic/cryptography';
...
export async function updateItem(
  ...
  try {
    ...
    if ('username' in updatedItem) {
      ...
      credentialsStore.updateCredential(itemId, updates as Partial<CredentialDecrypted>);
    } else if ('cardNumber' in updatedItem) {
      encryptedItem = await encryptBankCard(userSecretKey, updatedItem as BankCardDecrypted);
      bankCardsStore.updateBankCard(itemId, updates as Partial<BankCardDecrypted>);
    } else {
      encryptedItem = await encryptSecureNote(userSecretKey, updatedItem as SecureNoteDecrypted);
      secureNotesStore.updateSecureNote(itemId, updates as Partial<SecureNoteDecrypted>);
    }
...
export async function deleteItem(userId: string, itemId: string): Promise<void> {
  ...
  try {
    ...
    if (credentialIndex !== -1) {
      credentialsStore.deleteCredential(itemId);
    } else {
      const bankCardIndex = bankCardsStore.bankCards.findIndex((b: BankCardDecrypted) => b.id === itemId);
      if (bankCardIndex !== -1) {
        bankCardsStore.deleteBankCard(itemId);
      } else {
        secureNotesStore.deleteSecureNote(itemId);
      }
    }
