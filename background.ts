import { getCachedCredentialsByDomain } from './src/cache';
import { PageState, CachedCredential } from './src/types';
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  if (msg.type === 'INJECT_CREDENTIAL' && msg.credentialId && sender.tab && typeof sender.tab.id === 'number') {
    import('./src/cache').then(async ({ getCachedCredential }) => {
      const cred: CachedCredential | null = await getCachedCredential(msg.credentialId);
      if (cred) {
        // Decrypt password
  ...
  if (msg.type === 'GET_CACHED_CREDENTIALS' && msg.domain) {
    getCachedCredentialsByDomain(msg.domain).then(creds => {
      sendResponse(creds);
    });
    return true; // async
  }
